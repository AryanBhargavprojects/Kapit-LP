...
1. Tell the user to sign up at **https://kapit.dev/dashboard**
2. Create a `kap_live_...` (production) or `kap_test_...` (test) key
3. Set the environment variable `KAPIT_API_KEY`
...
Do NOT attempt: historical data, search, batch, streaming, options chains. Only the four GET endpoints above are available. Calling unsupported paths returns structured errors; do not retry them.
...
- `references/auth-and-billing.md` — API key setup, tiers, free tier, billing
